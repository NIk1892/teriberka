...
### Включение чата с сайта

Посетитель пишет в чат на сайте → сообщение в приватной группе менеджеров → менеджер
отвечает **reply** → ответ появляется на сайте. Бот и туннель — те же, что у отбивки.

1. Приватная группа менеджеров. **Сначала все настройки** (видимость истории, админы,
   темы), и только потом бот: такие настройки превращают группу в супергруппу с новым
   id, а диалоги запоминают старый.
2. Добавить бота обычным участником (privacy mode не трогать) → в
   `docker logs teriberka-api-chat-1` строка «Бота добавили в чат -100…».
3. Этот id → `TG_ADMIN_CHAT_ID` в `.env` → `docker compose ... up -d api-chat`.
4. В @BotFather: `/setjoingroups` → Disable, чтобы бота не добавляли в чужие группы.
5. Проверка: сообщение в чат на сайте → в группе шапка «💬 Новый чат с сайта · #…»,
   на сайте под сообщением «✓ Доставлено менеджеру» → reply в группе → 👍 от бота
   и ответ на сайте за ~3 с.

Недоставленное за время без `TG_ADMIN_CHAT_ID` или без туннеля уходит само: подметание
раз в минуту берёт сообщения моложе суток.

**Включено 16.09.2026**: супергруппа «TeriberkaKraySupportGroup», `TG_ADMIN_CHAT_ID=-1004345123980`
