...
- **Фото мест**: карточки на главной и большое фото страницы места — реальные снимки `wwwroot/img/places/{slug}.webp` (1600px, конвертация из Wikimedia Commons). Лицензии CC **требуют атрибуции**: автор/лицензия/ссылка лежат в `PlaceCatalog` (`PhotoCredit`), строка «Фото: …» на странице места — не удалять; при замене на собственные фотографии поле `Photo` у места обнуляется и подпись исчезает сама. Галерея страницы места — пока SVG-заглушка [img/placeholder.svg](src/frontend/ui.public.web/wwwroot/img/placeholder.svg) + бейдж «Фото скоро», разведена CSS-фильтрами по оттенку.
